...
## v0.9.1 本地就绪 Relay 服务器

- **自托管中继二进制** — `diskpulse-relay` 在 `127.0.0.1` 上启动本地 WebSocket 中继，用于自托管冒烟测试。
- **中继客户端 IPC** — `connect_relay`、`disconnect_relay`、`get_relay_status` 和 `list_cloud_devices` 将中继状态暴露给桌面应用。
- **只读路由守卫** — 中继信封复用 Hub 远程命令白名单，拒绝无本地确认的清理/写入命令。
- **本地就绪范围** — 公开中继部署、DNS/TLS 和真实跨 WAN 双设备验证仍为外部 CI/运维关卡。

## v0.9.0 完整智能

- **AE 异常检测基础** — 6D 快照特征、确定性 6→4→6 自编码器推理，以及合成训练样本。
- **文件分类器 Stage 3** — 8D 特征提取、扩展 magic 签名、12 类 softmax 分类输出与模型版本元数据。
- **文件类别风险规则** — 内置清理规则现在可匹配 `file_category` 条件，如 `dev_cache`、`build`、`dependency`。
- **外部存储检测** — 跨平台存储抽象层，Windows `WM_DEVICECHANGE` 事件模型，Linux/macOS 兜底实现与存储插拔 IPC 事件。
- **5 语言国际化** — 英文、简体中文、日语、韩语、西班牙语，系统语言自动识别。
- **AI 模型面板** — 设置 → AI Model 显示 AE/分类器版本、AUC/准确率指标、60 快照微调门槛与重置操作。
...
- **智能风险分类** — 19 条内置规则 + 自定义规则编辑器，将每个目录划分为低/中/高三个风险等级
...
- **国际化** — 5 种语言：英文 + 简体中文 + 日语 + 韩语 + 西班牙语，自动检测系统语言
...
|------|--------|
| 桌面框架 | Tauri 2.x |
| 后端 | Rust — 40 个源文件、6 个平台 trait、147 项测试 |
...
| **v0.9.0** | **完整智能：burn DL（AE + 分类器）、扩展存储、韩语/西班牙语** | ✅ Local |
| **v0.9.1** | **本地就绪 Relay 服务器（自托管二进制、IPC/状态、只读路由守卫）** | ✅ Local-ready |
...
trigger_fine_tune() -> ModelStatus
reset_model() -> ModelStatus

// 外部存储 (v0.8.6)
list_external_storage() -> Vec<ExternalStorageInfo>
get_storage_info(path: String) -> ExternalStorageInfo
